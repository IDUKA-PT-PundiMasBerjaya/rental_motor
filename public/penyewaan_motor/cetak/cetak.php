<?php
    ...
    $data = "SELECT penyewaan.id_penyewaan, customer.nama AS namapeminjam
                FROM penyewaan
                LEFT JOIN 
    ...
    $pdf->Cell(250, 10, "Data Peminjaman Buku - Peminjam: $nama_peminjam", 0, 0, 'R');
        ...
        foreach ($rowSpanCounts as $peminjamanID => $rows) {
            ...
            foreach ($rows as $key => $userAmbilData) {
                ...
                $pdf->Cell(30, 6 * $rowSpanCount, $userAmbilData['id_penyewaan'], 1, 0, 'C');
                    $pdf->Cell(50, 6 * $rowSpanCount, $userAmbilData['namapeminjam'], 1, 0, 'C');
                    $firstRow = false;
                } else {
